...
def filtering(G, GT, show_heatmaps=False, show_error=False):
    '''
    G: 2d-array representing map/graph
    GT: (tuple) such that GT=(C0, C, A, E) where
        C0 is initial location,
...
def viterbi(G, GT, N=10, show_trajectories=True, show_error=False):
    '''
    G: 2d-array representing map/graph
    GT: (tuple) such that GT=(C0, C, A, E) where
        C0 is initial location,
...
def display_trajectories(G, GT, MLS):
    ...
    rt = np.array(rt) + 0.50
    ct = np.array(ct) + 0.50
    plt.plot(ct, rt, 'b', label='Most Likely Sequence')
    ...
    plt.gca().xaxis.set_ticks_position('top')
    plt.legend(loc='best')
    plt.title('Most Likely Sequence', y=1.06, fontsize=12)
    ...
    plt.gca().xaxis.set_ticks_position('top')
    plt.legend(loc='best')
    plt.title('True Path', y=1.06, fontsize=12)
